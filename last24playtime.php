...
$s = $db->prepare('select p.n, count(p.i)*5/60 as time from t join p on t.i = p.i where t > unix_timestamp() - 86400 group by t.i order by time desc limit 20');
$s->execute();
$d = $s->fetchAll();
...
$settings = array(
  ...
  'graph_title' => 'most active players last 24h (h)',
